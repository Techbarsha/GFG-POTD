...
public:
	vector<int> maximumWeight(int n, vector<vector<int>> edges, int q, vector<int> &queries)
	{
        ...
        //sorting the edges based on their weights in ASC order
       ...
       //crating vector to store the results for each query
        vector<int> res;
        ...
        for(int i = 0; i < q; i++)
        {
            ...
            if(val == mp.begin())
                res.push_back(0); //if there is no such element,then maximum weight is 0
            else
            {
                val--;
